...
class EnumLevel2Field extends EnumField
{
    ...
    protected const FIELD_OPTIONS = 3;
    ...
    public function __construct(array $values, ContainerInterface $services)
    {
        ...
        $options=  explode('|', (!empty($values[self::FIELD_OPTIONS]) && is_string($values[self::FIELD_OPTIONS]) ? $values[self::FIELD_OPTIONS] : ''));
        $this->parentFieldName = trim($options[0] ?? '');
        $this->parentFormId = trim($options[1] ?? '');
        if (strval($this->parentFormId) != strval(intval($this->parentFormId))) {
            $this->parentFormId = '';
        }
    ...
    public function jsonSerialize()
    {
        if (!$this->internalField) {
            return [
                ...
                'parentFormId' => $this->getParentFormId()
                ];
        } else {
            ...
            $internalFieldData['parentFormId'] = $this->getParentFormId();
